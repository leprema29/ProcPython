...
def write_id_numero_mtn(file_path, identifications):
    """Write identification CSV for MTN"""
    with open(file_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Numero', 'NomPrenom', 'DateNaissance', 'NumeroCNI', 'DateExpCNI', 'Quartier', 'Nationalite'])
        for ident in identifications:
            writer.writerow([ident.numero, ident.nom_prenom, ident.date_naissance,
                           ident.numero_cni, ident.date_exp_cni, ident.quartier, ident.nationalite])


def write_listing_mtn(file_path, listings):
...
def write_statistiques_mtn(file_path, stats):
    """Write statistics CSV for MTN"""
    with open(file_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
...
def write_statistiques_localisation_mtn(file_path, stats):
    """Write location statistics CSV for MTN"""
    with open(file_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
...
def write_identification_abonnees_mtn(file_path, identifications):
    """Write subscriber identification CSV for MTN"""
    with open(file_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Numero', 'NomPrenom', 'DateNaissance', 'NumeroCNI', 'DateExpCNI', 'Quartier', 'Nationalite'])
        for ident in identifications:
